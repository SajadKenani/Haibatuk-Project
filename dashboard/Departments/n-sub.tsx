...
export const NESTEDSUB: React.FC = () => {
  ...
  const [deptToSend, setDeptToSend] = useState<NestedSubDepartment>({
    id: 0,
    name_en: "",
    name_ar: "",
    sub_department_id: localStorage.getItem("nestedDeptID") || "0",
  });
  const [showForm, setShowForm] = useState(false);
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState<Number | null>(null);

  const fetchNestedSubDepartment = async () => {
    try {
      const deptID = localStorage.getItem("nestedDeptID");
      const response = await POST("api/nested-sub-department-en", { sub_department_id: deptID });
      setNestedSubDepartment(response.data as NestedSubDepartment[]);
      setLoading(false);
    } catch (error) {
  ...
  useEffect(() => {
    fetchNestedSubDepartment();
  }, []);

  const sendNestedSubDepartment = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    try {
      const deptID = localStorage.getItem("nestedDeptID");
      await POST("api/nested-sub-department", deptToSend);
      toast.success("Nested Sub Department added successfully!");
      setDeptToSend({
        id: 0,
        name_en: "",
        name_ar: "",
        sub_department_id: deptID || "0",
      });
      setShowForm(false);
      await fetchNestedSubDepartment();
    } catch (error) {
  ...
  const removeSubDepartment = async (subDeptId: number) => {
    ...
    try {
      await DELETE("api/nested-sub-department", { id: subDeptId });
      toast.success("Nested Sub Department removed successfully!");
      await fetchNestedSubDepartment();
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">
            Nested-Sub-Departments Management
          </h1>
          <button
        ...
        ) : nestedSubDepartment?.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {nestedSubDepartment?.map((item) => (
              <div
                key={String(item.id)}
                className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 
                transition-all duration-300 hover:shadow-xl group relative"
              >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6 relative">
              ...
              <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">
                Add Nested-Sub-Department
              </h2>

              <form onSubmit={sendNestedSubDepartment} className="space-y-4">
                <div>
